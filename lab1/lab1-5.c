...
GLfloat vertices[18*3] =	// 18 straight lines, 3 is the number of points per cordinat (x, y, z). three lines represent one triangle.
{
	// base plate
...
void init(void)
{
	...
	glDisable(GL_DEPTH_TEST); // Activate the Z buffer and back-face culling
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "rotationMatrixY"), 1, GL_TRUE, rotationMatrixX);
...
void display(void)
{
	...
	// clear the screen (Erase the Z buffer before rendering)
	...
	rot_y = Rz(t/1000);		// y rotate
	rot_z = Rx(t/1000);		// z rotate
