...
// 220829: Changed filenames to const char to please g++.
...
int SaveDataToTGA(const char			*filename, 
			 short int		width, 
			 short int		height, 
...
void SaveTGA(TextureData *tex, const char *filename)
{
	SaveDataToTGA(filename, tex->width, tex->height, 
			tex->bpp, tex->imageData);
}

void SaveFramebufferToTGA(const char *filename, GLint x, GLint y, GLint w, GLint h)
{
	int err;
